...
type MockJobEvalLLM struct {
	mockResponse string
	boundTools   []*schema.ToolInfo // 可选：记录绑定的工具
}

// Generate 实现model.BaseChatModel接口 (也是ToolCallingChatModel的一部分)
...
// Stream 实现model.BaseChatModel接口 (也是ToolCallingChatModel的一部分)
...
func (m *MockJobEvalLLM) BindTools(tools []*schema.ToolInfo) error {
	m.boundTools = tools
	log.Printf("[MockJobEvalLLM] BindTools (deprecated) called with %d tools.", len(tools))
	return nil
}

// WithTools 实现 model.ToolCallingChatModel 接口
func (m *MockJobEvalLLM) WithTools(tools []*schema.ToolInfo) (model.ToolCallingChatModel, error) {
	log.Printf("[MockJobEvalLLM] WithTools called with %d tools.", len(tools))
	// 在这个mock中，我们简单地记录工具，然后返回自身，因为评估器不实际执行工具
	newMock := &MockJobEvalLLM{
		mockResponse: m.mockResponse, // 继承mock响应
		boundTools:   tools,          // 设置新的绑定工具
	}
	return newMock, nil
}
...
func checkScoreInRange(t *testing.T, score int, expectedRange string) bool {
	...
	minResp, err1 := strconv.Atoi(parts[0])
	maxResp, err2 := strconv.Atoi(parts[1])
	if err1 != nil || err2 != nil {
		t.Logf("警告: 无效的分数范围值: %s", expectedRange)
		return false
	}

	inRange := score >= minResp && score <= maxResp
	if !inRange {
...
func initRealLLMForJobEvaluatorTest(ctx context.Context) (model.ToolCallingChatModel, error) {
	...
	if configPath == "" {
		return nil, fmt.Errorf("未找到配置文件 (config.yaml)。请确保配置文件在项目路径中。")
	}
	...
	if apiKey == "" || apiKey == "your_api_key_here" || apiKey == "test_api_key" {
		return nil, fmt.Errorf("未在配置文件 %s 中找到有效的API Key。", configPath)
	}

	// 4. 设置默认值 (如果配置文件中也没有提供)
	if apiURL == "" {
...
func findConfigFile() string {
	// 在常见位置查找配置文件
	searchPaths := []string{
		"../config/config.yaml", // Preferred path from internal/parser
		"config.yaml",
		"./config.yaml",
		"../config.yaml", // This would resolve to internal/config.yaml if CWD is internal/parser
		"../../config.yaml",
	}
