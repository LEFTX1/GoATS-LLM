	...
	mockResponse := `{
		...
		"chunks": [
			...
			},
			{
				...
				"type": "EDUCATION",
				"title": "教育经历",
				"content": "清华大学，计算机科学与技术，2018-2022"
			},
			{
				"chunk_id": 3,
				"resume_identifier": "张三_13800138000",
				"type": "SKILLS",
				"title": "专业技能",
				"content": "精通Java、Spring Boot，熟悉MySQL、Redis"
			}
		],
		"metadata": {
			"is_211": true,
...
负责用户模块和订单系统，优化数据库查询性能提升30%`
	...
	// 验证技能章节
	var skillsSection *ResumeSection
	for _, section := range sections {
		if section.Type == SectionSkills {
...
func TestRealResumeParsing(t *testing.T) {
	...
	pdfExtractor, err := NewEinoPDFTextExtractor(ctx)
	if err != nil {
		t.Fatalf("创建PDF提取器失败: %v", err)
	}
	...
	for _, path := range testPDFs {
		if _, err := os.Stat(path); err == nil {
			// 提取PDF文本
			text, _, err := pdfExtractor.ExtractFullTextFromPDFFile(ctx, path)
			if err != nil {
				t.Logf("无法从%s提取文本: %v", path, err)
				continue
			}
			pdfText = text
			foundFile = true
			t.Logf("成功从PDF文件提取了%d个字符: %s", len(pdfText), path)
			break
		}
	}

	if !foundFile || pdfText == "" {
		t.Skip("找不到测试PDF文件或提取失败，跳过测试")
		return
	}

	//格式化打印pdfText
	t.Logf("提取的PDF文本: %s", pdfText)
	...
	_, فونFound := basicInfo["phone"]
	_, emailFound := basicInfo["email"]
	assert.True(t, فونFound || emailFound, "应至少提取电话或邮箱中的一个")
}
...
| 数据水平分片：核心业务表按CRC32哈希分16片，实现千万级数据高效存储与查询`
	...
	// 验证章节
	expectedSectionTypes := []SectionType{SectionEducation, SectionSkills, SectionProjects}
	foundSections := make(map[SectionType]bool)
...
func TestLLMWithDataAnalystResume(t *testing.T) {
	...
	// 数据分析师简历文本
	resumeText := `王浩宇15800009999wang.haoyu.cv@163.com个人主页: https://haoyu.wang.blog | GitHub: @hywang_dev求职意向: 数据分析师 / 商业智能 BI 实习生 地点: 杭州优先, 上海可考虑教育背景浙江大学 (ZJU) - 985 & 211 工程院校金融学 (辅修计算机科学) 本科学位2021年9月 至 2025年6月 (预计)主要课程: 统计学原理 (95/100), Python数据分析 (92/100), 机器学习导论 (88/100), 计量经济学, 金融市场学。校级优秀学生干部, 获得二等学业奖学金。实习经历ABC科技有限公司 - 数据分析实习生 (远程)2024年3月 – 2024年5月- 参与公司电商平台用户行为数据分析项目，协助收集和清洗来自不同渠道的原始数据 (SQL, Excel)。- 使用 Python (Pandas, NumPy, Matplotlib, Seaborn) 对用户购买路径、转化率等关键指标进行探索性数据分析 (EDA)，并撰写分析报告。- 协助构建用户画像标签体系，通过 RFM 模型对用户进行分层，为精准营销活动提供数据支持。- 每周向团队汇报分析进展，并根据反馈迭代分析方案。XYZ咨询公司 - 商业分析助理 (暑期实习)2023年7月 – 2023年8月- 协助咨询顾问进行市场调研，收集特定行业的竞争格局、市场规模等信息。- 参与撰写行业分析报告的部分章节，主要负责数据图表的制作 (Excel, PowerPoint)。- 学习并应用SWOT分析、波特五力模型等商业分析工具。专业技能数据分析 & 可视化:熟练掌握 Python (Pandas, NumPy, Scikit-learn) 进行数据处理、建模与分析。精通 SQL 进行复杂数据查询与提取 (MySQL, PostgreSQL)。能够使用 Tableau 或 Power BI 进行交互式数据可视化和仪表盘制作。熟悉常用的统计学方法和机器学习算法 (如回归、分类、聚类)。编程与其他:了解 Java 语言基础。英语 CET-6 (580分)，具备良好的英文文献阅读和沟通能力。熟练使用 Microsoft Office 套件 (Excel, Word, PowerPoint)。荣誉奖项- 2023年 全国大学生数学建模竞赛 省级二等奖- 2022年 浙江大学“挑战杯”创业计划大赛 校级铜奖- 2021-2023连续两年 浙江大学优秀学生二等奖学金自我评价对数据敏感，逻辑思维清晰，具备较强的学习能力和解决问题的能力。注重细节，有良好的沟通协调能力和团队合作精神。渴望在数据驱动决策的领域贡献自己的力量。`
	...
	// 验证章节
	expectedSectionTypes := []SectionType{SectionEducation, SectionSkills, SectionInternships, SectionAwards}
	foundSections := make(map[SectionType]bool)
...
func TestLLMWithMixedExperienceResume(t *testing.T) {
	...
	resumeText := `张伟13587654321zhang.wei.pro@GMAIL.COM上海 | 3年工作经验 | 高级软件工程师教育背景复旦大学 | 计算机软件与理论 | 硕士 | 2017-09 -- 2020-06工作经验BBD 技术有限公司 | 高级后端工程师 | 2021-07 -- 至今- 负责电商交易核心系统的设计与开发，采用微服务架构 (Spring Cloud, Dubbo)。- 主导订单履约模块重构，引入领域驱动设计(DDD)，系统QPS提升50%，故障率降低30%。- 指导初级工程师，进行代码评审和技术分享。实习经历CCA 科技有限公司 | 后端开发实习生 | 2019-07 -- 2019-09- 参与内容管理系统后端API开发，使用 Spring Boot 和 Mybatis。- 协助完成单元测试和接口文档编写。专业技能- 深入理解Java，熟悉JVM原理、并发编程、网络编程。- 精通Spring Boot, Spring Cloud, Dubbo等微服务框架。- 熟练使用MySQL, Oracle数据库，有SQL优化经验。- 熟悉Redis, Kafka, Elasticsearch等常用中间件。- 了解Docker, Kubernetes等容器化技术。`
	...
	t.Logf("\n【元数据分析】:")
	t.Logf("----------------------------------------------------------------------")
	metaFields := []string{"is_211", "is_985", "is_double_top", "has_intern", "highest_education", "years_of_experience", "resume_score", "tags"}
	...
	expectedSectionTypes := []SectionType{SectionEducation, SectionWorkExperience, SectionInternships, SectionSkills}
	foundSections := make(map[SectionType]bool)
	for _, section := range sections {
		foundSections[section.Type] = true
		// 检查实习和工作经验是否有内容
		if section.Type == SectionInternships {
			assert.NotEmpty(t, section.Content, "实习经历(INTERNSHIPS)章节不应为空")
		}
		if section.Type == SectionWorkExperience {
...
func initRealLLM(ctx context.Context) (model.ChatModel, error) {
	...
	log.Printf("尝试直接加载配置文件: %s (绝对路径: %s)", configPath, absConfigPath)
	...
	if _, err := os.Stat(configPath); err == nil {
		...
		if err != nil {
			...
			log.Printf("成功读取配置文件，大小: %d 字节", len(content))
		...
		log.Printf("配置文件不存在: %s, 错误: %v", configPath, err)
	}

	// 然后，允许环境变量覆盖配置文件中的值或提供缺失值
	envApiKey := os.Getenv("LLM_API_KEY")
	if envApiKey != "" {
		apiKey = envApiKey
		log.Printf("使用环境变量 LLM_API_KEY 覆盖 API Key")
	} else {
		log.Printf("环境变量 LLM_API_KEY 未设置")
	}

	envApiURL := os.Getenv("LLM_API_URL")
	if envApiURL != "" {
		apiURL = envApiURL
		log.Printf("使用环境变量 LLM_API_URL (%s) 覆盖 API URL", apiURL)
	}

	envModelName := os.Getenv("LLM_MODEL_NAME")
	if envModelName != "" {
		modelName = envModelName
		log.Printf("使用环境变量 LLM_MODEL_NAME (%s) 覆盖 Model Name", modelName)
	}

	// 检查API Key是否配置
	if apiKey == "" || apiKey == "your_api_key_here" { // 检查是否为空或为示例值
		return nil, fmt.Errorf("API Key 未配置。请在 config.yaml 或 LLM_API_KEY 环境变量中设置有效的API密钥。")
	}

	// 如果模型名称为空，设置一个默认值
	if modelName == "" {
		modelName = "qwen-plus" // 或其他合适的默认模型
		log.Printf("LLM 模型名称未配置，使用默认值: %s", modelName)
	}

	// apiURL 如果为空，NewAliyunQwenChatModel 内部会使用其默认值

	log.Printf("初始化 AliyunQwenChatModel: Model=%s, URL=%s", modelName, apiURL)
